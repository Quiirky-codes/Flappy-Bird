...
class Pipe:                    #pipes in game
    ...
    def collide(self, bird): #collision function
...
class Base: #adding the base image
    VEL =5
    ...
    def move(self):
        self.x1 -= self.VEL
        self.x2 -= self.VEL
        #if it reaches the end of the screen reset it to the start
...
def main(genomes, config):
    ...
    while run:
        ...
        rem = []
        for pipe in pipes:
            for x, bird in enumerate(birds):#pipe move function
        ...
        for x, bird in enumerate(birds): #bird out
